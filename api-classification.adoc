...
[role="lead"]
Usa la API de REST de NetApp Data Classification para acceder a las mismas capacidades que están disponibles en la interfaz de usuario web.
...
+
Por ejemplo, puedes localizar archivos _sin_ información de identificación personal (PII) específica (funcionalidad no disponible en la interfaz de usuario). También puedes excluir campos específicos para la operación de exportación.
...
Para acceder a Swagger, necesitas la dirección IP de tu instancia de NetApp Data Classification. Para una implementación en la nube, usa la dirección IP pública. Luego necesitas acceder a este endpoint:
...
.Solicitud de API
Necesitas obtener todos los campos y opciones relevantes para que un sistema muestre todos los filtros en la pestaña de Investigación.
...
----
Utiliza la respuesta de los parámetros de la solicitud para filtrar los archivos que deseas copiar.

Puede aplicar una acción a varios elementos.  Los tipos de acciones admitidos incluyen: mover, eliminar y copiar.

Crea la acción de copiar:

.Solicitud de API
Esta API te permite crear varias acciones.
