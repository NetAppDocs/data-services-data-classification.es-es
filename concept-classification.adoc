...
[role="lead"]
NetApp Data Classification es un servicio de gobernanza de datos en NetApp Console que analiza tus fuentes de datos locales y en la nube para mapear y clasificar los datos e identificar información privada. Te ayuda a reducir los riesgos de seguridad y de incumplimiento de normativas, a disminuir los costes de almacenamiento y a apoyar los proyectos de migración de datos.


IMPORTANT: A partir de la versión 1.31, Data Classification está disponible como una función básica dentro de la NetApp Console. No hay ningún cargo adicional. No se requiere ninguna licencia ni suscripción de Data Classification. + Si has estado usando la versión anterior 1.30 o una anterior, esa versión estará disponible hasta que expire tu suscripción.
...
La consola NetApp ofrece una gestión centralizada de los servicios de almacenamiento y datos de NetApp en entornos locales y en la nube a escala empresarial. La consola es necesaria para acceder y utilizar los servicios de datos de NetApp. Como interfaz de gestión, te permite gestionar numerosos recursos de almacenamiento desde una única interfaz. Los administradores de la consola pueden controlar el acceso al almacenamiento y a los servicios de todos los sistemas de la empresa.
...
* Identifica la información de carácter personal (PII).
...
Los costes de transferencia de datos dependen de tu configuración. Si la instancia de Data Classification y la fuente de datos están en la misma zona de disponibilidad y región, entonces no hay costes de transferencia de datos. Pero si la fuente de datos, como un sistema Cloud Volumes ONTAP, está en una zona de disponibilidad o región _diferente_, tu proveedor de servicios en la nube te cobrará los costes de transferencia de datos. Consulta estos enlaces para más detalles:
...
* En AWS, la clasificación de datos se ejecuta en un https://aws.amazon.com/ec2/instance-types/m6i/["instancia m6i.4xlarge"^] con un disco GP2 de 500 GiB. La imagen del sistema operativo es Amazon Linux 2.
...
* **Los análisis completos** ofrecen un análisis en profundidad de tus datos. Un análisis completo incluye un análisis exclusivo de la estructura de los archivos y la clasificación de los datos que contienen.


Para ver un desglose de las diferencias entre las exploraciones de sólo mapa y las completas, consulta link:task-scanning-overview.html["¿Cuál es la diferencia entre los escaneos de mapeo y clasificación?"].
...
* *Categorías*: La clasificación de datos toma los datos que escanea y los divide en diferentes tipos de categorías. Las categorías son temas basados en el análisis de IA del contenido y los metadatos de cada archivo. link:task-controlling-governance-data.html["Obtenga más información sobre las categorías"^].
