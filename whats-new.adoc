...
summary: Conozca las novedades en NetApp Data Classification. 
---
...
Esta versión de clasificación de BlueXP incluye cambios en el código que optimizan la utilización de recursos.
...
=== Dar prioridad a los análisis solo de mapas

Al realizar escaneos solo Map-only, puedes priorizar los escaneos más importantes. Esta función ayuda cuando tienes muchos entornos de trabajo y quieres asegurarte de que los escaneos de alta prioridad se completen primero.
...
La priorización se limita a link:https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["exploraciones solo de mapa"^]; no está disponible para las exploraciones de mapeo y clasificación.
...
Obtenga más información sobre https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["BlueXP classification"] .
...
Esta versión incluye pequeñas mejoras.

El lanzamiento también eliminó las siguientes opciones:

* Se ha deshabilitado la opción para activar la recopilación de registros de auditoría.
* Durante la investigación de Directorios, la opción de calcular el número de datos de información personal identificable (PII) por Directorios no está disponible. Consulta link:task-investigate-data.html["Investigue los datos almacenados en su organización"].
...
Las versiones 8 y 9 de Red Hat Enterprise Linux no son compatibles con el motor Docker, que anteriormente era necesario para la instalación de BlueXP classification. BlueXP classification ahora es compatible con la instalación en RHEL 9.0, 9.1 y 9.2 usando Podman versión 4 o superior como infraestructura de contenedores. Si tu entorno requiere usar las versiones más recientes de RHEL, ahora puedes instalar BlueXP classification (versión 1.26 o superior) cuando uses Podman.

En la actualidad, Data Classification no admite ni instalaciones en sitios oscuros ni entornos de escaneado distribuidos (utilizando un nodo maestro y nodos de escaneado remotos) cuando se utiliza RHEL 9.x.
...
=== Añade palabras clave personalizadas desde la interfaz de clasificación de BlueXP
...
El filtro "Número de usuarios con acceso" ahora admite archivos almacenados en repositorios de SharePoint Online. Anteriormente, solo se admitían archivos en recursos compartidos CIFS. Los grupos de SharePoint que no están basados en Active Directory no se tienen en cuenta en este filtro por el momento.
