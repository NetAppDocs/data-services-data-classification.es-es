...
---
= Instala NetApp Data Classification en un host Linux en modo privado
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Utiliza el _modo privado_ para instalar NetApp Data Classification en un host Linux en un sitio local sin acceso a internet (también conocido como instalación sin conexión o dark site). Esta implementación utiliza un script de instalación y mantiene la instancia de Data Classification desconectada de la capa SaaS de la NetApp Console, lo que te permite analizar fuentes de datos locales en un entorno aislado.
...
El script de instalación de Data Classification comprueba si el sistema y el entorno cumplen los requisitos previos necesarios. Si es así, comienza la instalación. Si quieres verificar los requisitos previos sin ejecutar la instalación de Data Classification, puedes descargar un paquete de software independiente que solo prueba los requisitos previos. link:task-test-linux-system.html["Vea cómo comprobar si su host Linux está listo para instalar la Clasificación de Datos"].
...
Cuando se instala en modo privado, NetApp Data Classification solo puede analizar datos procedentes de fuentes de datos que también se encuentren en el entorno local. En este momento, NetApp Data Classification puede analizar las siguientes fuentes de datos *locales*:
...
No se admite la migración del agente de la Console a otro host cuando NetApp Data Classification está implementado en un agente de la Console. Si necesitas Classification en otro agente de la Console en un sitio dark, implementa un nuevo agente de la Console y una nueva instancia de NetApp Data Classification en el nuevo host.
...
* La clasificación de datos debe realizarse en un host dedicado. El host no puede compartirse con otras aplicaciones ni con software de terceros, como antivirus.
...
** *Tamaño de VM de Azure*: "Standard_D16_v5". link:reference-instance-types.html#azure-instance-types["Ver tipos de instancias de Azure adicionales"^].
...
* *Gestión de suscripciones de Red Hat*: El servidor debe estar registrado en la Gestión de suscripciones de Red Hat. Si no está registrado, el sistema no puede acceder a los repositorios para actualizar el software de terceros necesario durante la instalación.
...
NOTE: El instalador de Data Classification reserva el UID 4321 y el GID 4321. Antes de iniciar la instalación, asegúrate de que el UID 4321 y el GID 4321 no estén en uso en el host. Si están en uso, la instalación fallará con un mensaje de error de "usuario no válido".
...
En la mayoría de los casos, instalas Data Classification en un sistema de un solo host. Esto no es el mismo host de máquina virtual que el agente de la consola.
