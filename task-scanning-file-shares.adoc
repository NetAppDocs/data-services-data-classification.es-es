...
Puedes iniciar o detener los análisis en cualquier sistema en cualquier momento desde el menú de configuración. También puedes cambiar los análisis de solo mapa a completos, y viceversa.

La opción *Escanear sin permisos de escritura* está desactivada por defecto. Esto significa que, si NetApp Data Classification no tiene los permisos de escritura adecuados, el sistema no escaneará los archivos porque NetApp Data Classification no puede restablecer el «tiempo de último acceso» a la marca de tiempo original. Si tu sistema puede tolerar que se modifique el tiempo de último acceso, selecciona esta opción. link:reference-collected-metadata.html#last-accessed-time-timestamp["Más información"^].


NOTE: Los nuevos volúmenes añadidos al sistema solo se analizan automáticamente cuando has activado el análisis para todos los volúmenes. Si solo has activado el análisis en ciertos volúmenes, debes activar manualmente el análisis en el volumen recién añadido.
