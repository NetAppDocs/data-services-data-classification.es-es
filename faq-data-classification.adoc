...
La exploración de sólo mapa no calienta los datos fríos: se mantienen fríos y permanecen en el almacenamiento de objetos. Por otro lado, si realizas la exploración completa, algunas configuraciones podrían calentar los datos fríos.
...
* *Escaneado profundo de clasificación de datos (escaneado completo)*: Clasificación de datos escanea los datos utilizando protocolos estándar y permisos de solo lectura en todos tus entornos. Los archivos seleccionados se abren y se escanean en busca de datos confidenciales relacionados con la empresa, información privada y problemas relacionados con ransomware.
+
...
Además, si desea permitir que ciertos usuarios solo vean los resultados del análisis de Clasificación de datos sin tener la capacidad de administrar las configuraciones de Clasificación de datos, puede asignar a esos usuarios el rol de *Visor de clasificación* (cuando use la NetApp Console en modo estándar) o el rol de *Visor de cumplimiento* (cuando use la NetApp Console en modo restringido). link:concept-classification.html["Más información"] .
